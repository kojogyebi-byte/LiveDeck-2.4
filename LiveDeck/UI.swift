...
struct MainView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $engine.showWelcome) {
            WelcomeView().environmentObject(engine).environmentObject(present)
        }
        .sheet(isPresented: $engine.showZoomMeeting) {
            ZoomMeetingPanel().environmentObject(engine)
        }
    ...
    private func handleDrop(_ providers: [NSItemProvider]) -> Bool {
        ...
        for p in providers where p.hasItemConformingToTypeIdentifier(UTType.fileURL.identifier) {
            ...
            p.loadItem(forTypeIdentifier: UTType.fileURL.identifier, options: nil) { item, _ in
                ...
                if let url { DispatchQueue.main.async { FileAccess.remember(url); engine.addDroppedFile(url) } }
            }
        }
        return accepted
    }
...
struct SystemStatsView: View {
    ...
    @ViewBuilder func stat(_ label: String, _ v: Double) -> some View {
        HStack(spacing: 4) {
            Text(label).font(.system(size: 9.5, weight: .semibold)).foregroundColor(.secondary).lineLimit(1).fixedSize()
            Text("\(Int(v.rounded()))%").font(.system(size: 10, weight: .semibold).monospacedDigit())
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            .contextMenu {
                ...
                Toggle("Record each input to its own audio track", isOn: $engine.recordSeparateTracks)
                Button("Export last recording as podcast audio (.m4a)") { engine.exportPodcastAudio() }.disabled(engine.lastRecordingURL == nil)
            ...
            Menu {
                ...
                Button("Choose recording folder…") { engine.chooseOutputFolder() }
                Button("Reveal last recording") { engine.revealLastRecording() }
                checkButton("Record each input to its own audio track", engine.recordSeparateTracks) { engine.recordSeparateTracks.toggle() }
                Button("Export a recording as podcast audio (.m4a)…") { engine.chooseRecordingForPodcastExport() }
                Divider()
                Button("Language & setup assistant…") { engine.showWelcome = true }
            ...
            HStack(spacing: 3) {
                VStack(spacing: 2) {
                    ForEach([0, -6, -12, -20, -30, -40, -60], id: \.self) { db in
                        Text("\(db)").font(.system(size: 7).monospacedDigit()).foregroundColor(.white.opacity(0.7))
        ...
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                DSTabBar(selection: $present.deck, items: [
                    ...
                    DSTabItem(id: DeckTab.atem.rawValue, title: "ATEM", icon: "rectangle.3.group")
                ])
            ...
            else if present.deck == DeckTab.atem.rawValue { ATEMDeck() }
            else { InputBus() }
        }
        .background(DS.bg1)
    }
...
struct AddInputMenuItems: View {
    ...
    var body: some View {
        ...
        Button("Zoom Meeting — built in (participants as inputs)…") { later { engine.showZoomMeeting = true } }
        Button("NDI® Source…") { later { engine.showNDIPicker = true } }
        Menu("Blackmagic DeckLink / UltraStudio") {
            let inputs = engine.deckLink.inputDevices
            if !engine.deckLink.apiPresent { Text("Install Blackmagic Desktop Video first") }
            else if inputs.isEmpty { Text("No DeckLink capture device found") }
            ForEach(inputs) { d in Button(d.name) { place(DeckLinkSource(device: d)) } }
            Divider()
            Button("Refresh devices") { engine.deckLink.refresh() }
        }
        Divider()
        ...
        Divider()
        Button("Sound Pads (jingles, intros, stingers)") {
            let s = SoundPadsSource(); place(s); engine.selectedSourceID = s.id; engine.rightTab = 1
        }
        Divider()
...
struct RightPanel: View {
    ...
    static let allTabs: [DSTabItem] = [
        DSTabItem(id: 1, title: "Input", icon: "rectangle.and.hand.point.up.left"),
        DSTabItem(id: 7, title: "Format", icon: "paintbrush.pointed"),
    ...
    var body: some View {
        let visible = Self.allTabs.filter { !engine.hiddenRightTabs.contains($0.id) }
        let hidden = Self.allTabs.filter { engine.hiddenRightTabs.contains($0.id) }
        VStack(spacing: 0) {
            HStack(spacing: 4) {
                Text("CONTROL PANEL").font(CPFont.section).kerning(0.6).foregroundColor(CP.text2)
                Spacer()
                Menu {
                    Section("Tabs") {
                        ForEach(Self.allTabs) { tab in
                            Button {
                                if engine.hiddenRightTabs.contains(tab.id) { engine.hiddenRightTabs.remove(tab.id); engine.rightTab = tab.id }
                                else { engine.hideRightTab(tab.id) }
                            } label: {
                                if engine.hiddenRightTabs.contains(tab.id) { Text(tab.title) } else { Label(tab.title, systemImage: "checkmark") }
                            }
                        }
                    }
                    Divider()
                    Button(engine.rightTabLabels ? "Icons only (smaller tabs)" : "Show tab names") { engine.rightTabLabels.toggle() }
                    if !hidden.isEmpty { Button("Show all tabs") { engine.hiddenRightTabs = [] } }
                } label: {
                    Image(systemName: hidden.isEmpty ? "square.grid.2x2" : "square.grid.2x2.fill").font(.system(size: 11, weight: .medium))
                        .foregroundColor(hidden.isEmpty ? CP.text2 : CP.text)
                }
                .menuStyle(.borderlessButton).menuIndicator(.hidden).fixedSize()
                .help(hidden.isEmpty ? "Choose which tabs to show" : "\(hidden.count) tab\(hidden.count == 1 ? "" : "s") hidden — click to bring back")
                Rectangle().fill(CP.divider).frame(width: 1, height: 14).padding(.horizontal, 2)
            ...
            CPTabBar(selection: $engine.rightTab, items: visible, showLabels: engine.rightTabLabels,
                     onClose: visible.count > 1 ? { engine.hideRightTab($0) } : nil)
                .padding(.horizontal, 10).padding(.top, 10).padding(.bottom, 4)
            if !hidden.isEmpty {
                HStack(spacing: 6) {
                    ForEach(hidden) { tab in
                        Button { engine.hiddenRightTabs.remove(tab.id); engine.rightTab = tab.id } label: {
                            HStack(spacing: 3) {
                                Image(systemName: "plus").font(.system(size: 8, weight: .bold))
                                Text(tab.title).font(.system(size: 10.5, weight: .medium))
                            }
                            .foregroundColor(CP.text2).padding(.horizontal, 7).frame(height: 20)
                            .background(Capsule().strokeBorder(CP.border, lineWidth: 1))
                        }
                        .buttonStyle(.plain).help("Bring back the \(tab.title) tab")
                    }
                    Spacer()
                }
            ...
            Group {
                ...
                else if engine.rightTab == 7 { FormatPanel() }
                else { OutputsPanel() }
            }
        ...
        .onAppear {
            if engine.hiddenRightTabs.contains(engine.rightTab), let first = visible.first { engine.rightTab = first.id }
        }
    }
...
/// Control-panel Format tab: the look editor of the slide deck in use (Songs & Bible, Dictionary or AI Search).
struct FormatPanel: View {
    @EnvironmentObject var present: PresentModel
    @EnvironmentObject var engine: Engine

    var body: some View {
        let deck = [DeckTab.present.rawValue, DeckTab.dictionary.rawValue, DeckTab.ai.rawValue].contains(present.deck) ? present.deck : present.lastFormatDeck
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                Text("FORMAT FOR").font(CPFont.section).kerning(0.6).foregroundColor(CP.text2)
                DSSegmented(selection: Binding(get: { deck }, set: { v in present.lastFormatDeck = v; present.deck = v }),
                            options: [(DeckTab.present.rawValue, "Songs & Bible"), (DeckTab.dictionary.rawValue, "Dictionary"), (DeckTab.ai.rawValue, "AI Search")])
            }
            .padding(.horizontal, 12).padding(.vertical, 6)
            Group {
                if deck == DeckTab.dictionary.rawValue { LookColumn(dictionary: true, showHeader: false) }
                else if deck == DeckTab.ai.rawValue { LookColumn(dictionary: false, ai: true, showHeader: false) }
                else { LookColumn(dictionary: false, showHeader: false) }
            }
            .id(deck)
        }
        .background(CP.bg)
        .onAppear { if deck != present.lastFormatDeck { present.lastFormatDeck = deck } }
        .onChange(of: present.deck) { d in
            if [DeckTab.present.rawValue, DeckTab.dictionary.rawValue, DeckTab.ai.rawValue].contains(d) { present.lastFormatDeck = d }
        }
    }
}
...
struct InputSettingsPanel: View {
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(spacing: 12) {
                ...
                if let s = engine.sources.first(where: { $0.id == engine.selectedSourceID }), !s.isPlaceholder {
                    ...
                    if let sp = s as? SoundPadsSource { SoundPadsCard(source: sp).id("sp-" + s.id.uuidString) }
                    ...
                    .background(RoundedRectangle(cornerRadius: 6).fill(CP.card))
                    .overlay(RoundedRectangle(cornerRadius: 6).strokeBorder(CP.border, lineWidth: 1))
                }

                HStack(spacing: 6) {
                    Image(systemName: "network").font(.system(size: 11, weight: .medium)).foregroundColor(CP.text2)
                    Text("NETWORK").font(CPFont.section).kerning(0.6).foregroundColor(CP.text2)
        ...
        .background(CP.bg)
        .onChange(of: engine.scrollInputToNetwork) { go in
            guard go else { return }
            withAnimation(.easeInOut(duration: 0.25)) { proxy.scrollTo("network", anchor: .top) }
            engine.scrollInputToNetwork = false
        }
        .onAppear {
            if engine.scrollInputToNetwork {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { proxy.scrollTo("network", anchor: .top); engine.scrollInputToNetwork = false }
            }
        }
        }
    }
...
struct PlaybackTransport<S: MediaPlayback>: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            HStack {
                Text(tc(scrubbing ? scrubValue : source.currentTime)).font(.system(size: 9).monospacedDigit()).foregroundColor(.secondary)
                Spacer()
                Text(tc(source.duration)).font(.system(size: 9).monospacedDigit()).foregroundColor(.secondary)
            }
            HStack(spacing: 10) {
...
struct ClockText: View {
    @EnvironmentObject var tele: Telemetry
    var body: some View {
        Text(tele.clock).font(.system(size: 14, weight: .bold).monospacedDigit()).foregroundColor(cProgram)
    }
}
...
struct FPSText: View {
    @EnvironmentObject var tele: Telemetry
    var body: some View {
        Text("FPS \(tele.fps)").font(.system(size: 10).monospacedDigit()).foregroundColor(.secondary)
    }
}
...
struct ChannelDBLabel: View {
    ...
    var body: some View {
        ...
        Text(dbReadout(lvl)).font(.system(size: 9, weight: .bold).monospacedDigit())
...
struct BusDBLabel: View {
    ...
    var body: some View {
        ...
        Text(dbReadout(lvl)).font(.system(size: 10, weight: .bold).monospacedDigit())
            .foregroundColor(meterDB(lvl) >= -2 ? vmRed : .secondary)
    }
}
        ...
        if source.isPlaceholder {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(gainDBText(source.gain)).font(.system(size: 8).monospacedDigit())
        ...
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                ForEach(marks, id: \.self) { m in
                    ...
                    Text("\(m)").font(.system(size: 7).monospacedDigit()).foregroundColor(.secondary)
            ...
            if let g = freeGB {
                HStack(spacing: 4) {
                    Image(systemName: g < 5 ? "externaldrive.badge.exclamationmark" : "externaldrive")
                        .font(.system(size: 9))
                    Text(String(format: "%.0f GB free", g)).font(.system(size: 10).monospacedDigit())
                }
                .foregroundColor(g < 5 ? .red : .secondary)
                .help(g < 5 ? "Low disk space on the recording volume" : "Free space on the recording volume")
            } else {
                Text("Disk —").font(.system(size: 10).monospacedDigit()).foregroundColor(.secondary)
            }
        }
...
struct OutputsQuickPanel: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !engine.deckLink.outputDevices.isEmpty {
                row("DeckLink", engine.deckLink.outputActive ? engine.deckLink.outputMode : "Off", on: engine.deckLink.outputActive) {
                    if engine.deckLink.outputActive { engine.deckLink.stopOutput() }
                    else { engine.deckLink.startOutput(width: engine.width, height: engine.height, format: engine.frameFormat) }
                }
            }
            if vcam.installState == .installed {
                row("LiveDeck Camera", vcam.sending ? (vcam.connected ? "Sending to Zoom / Teams" : "Connecting") : "Off", on: vcam.sending) { vcam.sending.toggle() }
            }
            CPDivider()
...
struct StreamRow: View {
    ...
    var body: some View {
        CPCard(title: dest.name.isEmpty ? "Destination" : dest.name, subtitle: dest.enabled ? "\(dest.platform) · enabled" : "\(dest.platform) · off",
            ...
            HStack {
                Text(dest.composedURL).font(.system(size: 9).monospacedDigit()).foregroundColor(CP.text2).lineLimit(1).truncationMode(.middle)
                Spacer()
            ...
            if mode == 4 {
                ...
                HStack {
                    ...
                    Text(peakMbps < 0.1 ? "Auto" : String(format: "%.0f Mbps", peakMbps))
                        .font(.system(size: 10).monospacedDigit()).frame(width: 60, alignment: .trailing)
                }
...
func pickFile(types: [String], completion: @escaping (URL) -> Void) {
    ...
    panel.begin { resp in if resp == .OK, let url = panel.url { FileAccess.remember(url); completion(url) } }
}
