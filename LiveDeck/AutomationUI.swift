...
struct AutomationDeck: View {
    @AppStorage("automation.page") private var page = 0
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                DSSegmented(selection: $page, options: [(0, "Auto Mix"), (1, "Timed cues")]).frame(width: 240)
                Text(page == 0 ? "Switch automatically between your inputs — by time or by who is speaking."
                               : "Lower thirds, logos and keys at the right moment.")
                    .font(.system(size: 11)).foregroundColor(DS.text2)
                Spacer()
            }
            .padding(.horizontal, 10).frame(height: 38).background(DS.bg1)
            .overlay(Rectangle().fill(DS.lineSoft).frame(height: 1), alignment: .bottom)
            if page == 0 { AutoMixView() } else { AutomationCuesDeck() }
        }
    }
}

struct AutomationCuesDeck: View {
