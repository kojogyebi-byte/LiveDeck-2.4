...
struct OnAirStatusBar: View {
        ...
        HStack(spacing: 6) {
            ...
            chip {
                VStack(alignment: .leading, spacing: 0) {
                    Text(now.formatted(.dateTime.hour().minute().second())).font(.system(size: 15, weight: .bold).monospacedDigit()).foregroundColor(DS.text)
            ...
            HStack(spacing: 6) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    Text(engine.isRecording ? "REC " + StatusFormat.duration(tele.recordSeconds) : "REC OFF")
                        .font(.system(size: 11.5, weight: .bold).monospacedDigit()).foregroundColor(engine.isRecording ? DS.program : DS.text3)
        ...
        return chip(tint: engine.isStreaming ? (h.level == .poor ? DS.program : DS.program.opacity(0.7)) : (failed ? DS.program : nil)) {
            HStack(spacing: 7) {
                ...
                if engine.isStreaming {
                    VStack(alignment: .leading, spacing: 0) {
                        Text(StatusFormat.duration(tele.streamSeconds)).font(.system(size: 11.5, weight: .bold).monospacedDigit()).foregroundColor(DS.text)
                        Text("\(StatusFormat.bitrate(tele.streamProgress.bitrateKbps)) · \(String(format: "%.2fx", tele.streamProgress.speed)) · \(tele.streamProgress.dropFrames) dropped")
                            .font(.system(size: 8.5, weight: .semibold).monospacedDigit()).foregroundColor(color)
                    }
    ...
    private func label(_ title: String, _ value: String, color: Color) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text(value).font(.system(size: 11.5, weight: .bold).monospacedDigit()).foregroundColor(color).lineLimit(1)
        }
    }
}
...
struct StreamDetailPopover: View {
    ...
    private func row(_ k: String, _ v: String) -> some View {
        GridRow {
            ...
            Text(v).font(.system(size: 11).monospacedDigit()).foregroundColor(CP.text)
        }
    }
}
    ...
    var body: some View {
        ...
        CPCard(title: "Quality & audio", subtitle: "\(out.width)×\(out.height) · video \(StreamBitrates.label(engine.streamBitrateKbps)) + audio \(engine.streamAudio ? StreamBitrates.label(engine.streamAudioBitrateKbps) : "off")", icon: "slider.horizontal.3") {
            ...
            HStack(alignment: .firstTextBaseline) {
                Text(StreamBitrates.label(total)).font(.system(size: 18, weight: .bold).monospacedDigit()).foregroundColor(CP.text)
                ...
                VStack(alignment: .trailing, spacing: 0) {
                    ...
                    Text("≈ \(StreamBitrates.label(upload))").font(.system(size: 12, weight: .semibold).monospacedDigit()).foregroundColor(CP.text)
                }
            }
            .padding(.vertical, 4)
