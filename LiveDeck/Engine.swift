...
final class Engine: ObservableObject {
    ...
    @Published var showZoomMeeting = false
    /// NDI® Program / Preview outputs (their own observable model).
    let ndiOutputs = NDIOutputs()
    /// LiveDeck Camera — virtual webcam for Zoom / Teams / Meet (Xcode builds with the Camera Extension).
    let virtualCamera = VirtualCameraManager()
    /// Blackmagic DeckLink / UltraStudio output and device list.
    let deckLink = DeckLinkManager()
    /// Blackmagic ATEM switcher control.
    let atem = ATEMSwitcher()
    /// Automatic switching between chosen inputs (Automation → Auto Mix).
    let autoMix = AutoMixController()
    /// Program loudness (LUFS) for podcasts, streaming and broadcast.
    let loudness = LoudnessMonitor()
    /// Record each input's audio to its own WAV file next to the recording.
    @Published var recordSeparateTracks: Bool = UserDefaults.standard.bool(forKey: "rec.tracks") {
        didSet { UserDefaults.standard.set(recordSeparateTracks, forKey: "rec.tracks") }
    }
    private var trackRecorder: MultitrackRecorder?
    ...
    func updatePowerGuard() {
        let busy = isStreaming || isRecording || streamReconnecting || programWindowActive || !activeScreens.isEmpty
            || ndiOutputs.programSender != nil || ndiOutputs.previewSender != nil || deckLink.outputActive
    ...
    func chooseOutputFolder() {
        ...
        panel.begin { [weak self] resp in if resp == .OK, let url = panel.url { FileAccess.remember(url); self?.outputFolderPath = url.path } }
    }
    ...
    @Published var rightTab = 0 {   // 0 Audio · 1 Input · 2 Overlays · 3 Scenes · 4 Outputs · 5 Presets · 6 Network (inside Input) · 7 Format
        didSet {
            if rightTab == 6 { rightTab = 1; hiddenRightTabs.remove(1); scrollInputToNetwork = true }
            else if hiddenRightTabs.contains(rightTab) { hiddenRightTabs.remove(rightTab) }
        }
    }
    /// Control-panel tabs the user closed (UserDefaults cp.hiddenTabs).
    @Published var hiddenRightTabs: Set<Int> = Set(UserDefaults.standard.array(forKey: "cp.hiddenTabs") as? [Int] ?? []) {
        didSet { UserDefaults.standard.set(Array(hiddenRightTabs), forKey: "cp.hiddenTabs") }
    }
    @Published var rightTabLabels: Bool = UserDefaults.standard.object(forKey: "cp.tabLabels") as? Bool ?? true {
        didSet { UserDefaults.standard.set(rightTabLabels, forKey: "cp.tabLabels") }
    }
    /// Set when something asks for the Network section, which now lives at the bottom of the Input tab.
    @Published var scrollInputToNetwork = false
    /// Tab to return to when leaving Songs & Bible / Dictionary / AI Search (which open the Format tab).
    var tabBeforeFormat: Int?

    static let rightTabOrder = [1, 7, 0, 2, 3, 4, 5]

    func hideRightTab(_ id: Int) {
        let visible = Engine.rightTabOrder.filter { !hiddenRightTabs.contains($0) }
        guard visible.count > 1 else { return }
        hiddenRightTabs.insert(id)
        if rightTab == id, let next = Engine.rightTabOrder.first(where: { !hiddenRightTabs.contains($0) }) { rightTab = next }
    }

    /// Lower-deck tab changed: slide decks show their Format in the control panel.
    func deckChanged(to deck: Int) {
        let formatDecks = [DeckTab.present.rawValue, DeckTab.dictionary.rawValue, DeckTab.ai.rawValue]
        if formatDecks.contains(deck) {
            guard !hiddenRightTabs.contains(7) else { return }
            if rightTab != 7 { tabBeforeFormat = rightTab; rightTab = 7 }
        } else if rightTab == 7, let back = tabBeforeFormat {
            tabBeforeFormat = nil
            rightTab = back
        }
    }
    ...
    func setResolution(width: Int, height: Int) {
        ...
        self.width = width; self.height = height; persistSettings()
        if deckLink.outputActive { deckLink.startOutput(width: width, height: height, format: frameFormat) }
    }
    ...
    func setFrameFormat(_ id: String) {
        ...
        persistSettings()
        if deckLink.outputActive { deckLink.startOutput(width: width, height: height, format: ff) }
        timer?.invalidate()
    ...
    private func renderFrame() {
        ...
        if virtualCamera.sending { virtualCamera.send(framePB ?? pb) }
        if deckLink.outputActive {
            if frameFormat.interlaced { if let frame = framePB { deckLink.send(frame) } } else { deckLink.send(pb) }
        }
    ...
    private func startRecording() {
        ...
        do {
            ...
            if recordSeparateTracks {
                let channels = sources.filter { !$0.isPlaceholder }.map { ($0.id, $0.name) }
                let folder = URL(fileURLWithPath: url.deletingPathExtension().path + " tracks")
                if let rec = MultitrackRecorder(folder: folder, channels: channels) {
                    trackRecorder = rec
                    audio.setTrackRecorder(rec)
                }
            }
    ...
    private func stopRecording() {
        ...
        if let rec = trackRecorder {
            audio.setTrackRecorder(nil)
            trackRecorder = nil
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { rec.finish() }
        }
    ...
    /// Show file last opened or saved (quick save when quitting).
    @Published var showURL: URL?
    @Published var showWelcome = !UserDefaults.standard.bool(forKey: "app.welcomeDone")

    func saveShow() { saveShow(completion: nil) }

    func saveShow(completion: ((Bool) -> Void)?, askForLocation: Bool = true) {
        ...
        if !askForLocation, let url = showURL {
            let ok = (try? data.write(to: url)) != nil
            completion?(ok)
            return
        }
        let panel = NSSavePanel(); panel.nameFieldStringValue = showURL?.lastPathComponent ?? "Untitled.livedeck"
        if let t = UTType(filenameExtension: "livedeck") { panel.allowedContentTypes = [t] }
        panel.begin { [weak self] resp in
            if resp == .OK, let url = panel.url, (try? data.write(to: url)) != nil {
