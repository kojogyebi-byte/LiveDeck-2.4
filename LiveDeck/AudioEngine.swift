    ...
    static func install(on item: AVPlayerItem, context: MediaTapContext, completion: @escaping (Bool) -> Void) {
        item.asset.loadTracks(withMediaType: .audio) { tracks, _ in
            DispatchQueue.main.async {
                ...
                guard let tap = makeProcessingTap(MTAudioProcessingTapCreate, &callbacks) else { completion(false); return }
...
final class MixChannel {
    ...
    var mL = [Float](), mR = [Float](), lL = [Float](), lR = [Float](), rL = [Float](), rR = [Float]()
...
final class ProgramAudioEngine {
    ...
    /// Separate track per channel while recording (set on the main thread, used by render).
    private var trackRecorder: MultitrackRecorder?
    func setTrackRecorder(_ r: MultitrackRecorder?) { withLock { trackRecorder = r } }
    ...
    private func render(frames n: Int, timestamp: UnsafePointer<AudioTimeStamp>, output: UnsafeMutablePointer<AudioBufferList>) {
        ...
        for (idx, c) in chans.enumerated() {
            ...
            peaks[idx] = (pkL, pkR)
            if let tracks, hasAudio {
                ensure(&c.rL, n); ensure(&c.rR, n)
                for i in 0..<n { c.rL[i] = c.mL[i] + c.lL[i]; c.rR[i] = c.mR[i] + c.lR[i] }
                c.rL.withUnsafeBufferPointer { l in c.rR.withUnsafeBufferPointer { r in tracks.write(c.id, l.baseAddress!, r.baseAddress!, n) } }
            }
        }
...
// MARK: - MTAudioProcessingTapCreate across SDKs
//
// Older macOS SDKs (Xcode 15) import the output parameter as UnsafeMutablePointer<Unmanaged<MTAudioProcessingTap>?>;
// newer SDKs import it as UnsafeMutablePointer<MTAudioProcessingTap?> (memory-managed). The function is passed in, so
// the compiler picks whichever overload matches the SDK in use — the same source builds with every Xcode.

private typealias TapCreateUnmanaged = (CFAllocator?, UnsafePointer<MTAudioProcessingTapCallbacks>, MTAudioProcessingTapCreationFlags,
                                        UnsafeMutablePointer<Unmanaged<MTAudioProcessingTap>?>) -> OSStatus
private typealias TapCreateManaged = (CFAllocator?, UnsafePointer<MTAudioProcessingTapCallbacks>, MTAudioProcessingTapCreationFlags,
                                      UnsafeMutablePointer<MTAudioProcessingTap?>) -> OSStatus

private func makeProcessingTap(_ create: TapCreateUnmanaged, _ callbacks: inout MTAudioProcessingTapCallbacks) -> MTAudioProcessingTap? {
    var tap: Unmanaged<MTAudioProcessingTap>?
    guard create(kCFAllocatorDefault, &callbacks, kMTAudioProcessingTapCreationFlag_PostEffects, &tap) == noErr else { return nil }
    return tap?.takeRetainedValue()
}

private func makeProcessingTap(_ create: TapCreateManaged, _ callbacks: inout MTAudioProcessingTapCallbacks) -> MTAudioProcessingTap? {
