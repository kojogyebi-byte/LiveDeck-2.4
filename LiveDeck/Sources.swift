...
final class StreamOutput {
    ...
    /// ffmpeg bundled inside the app (Contents/Helpers/ffmpeg — required for the Mac App Store edition),
    /// otherwise an installed copy (Homebrew / MacPorts).
    static func ffmpegPath() -> String? {
        ...
        if !AppEdition.isAppStore {
            candidates += ["/opt/homebrew/bin/ffmpeg", "/usr/local/bin/ffmpeg", "/usr/bin/ffmpeg", "/opt/local/bin/ffmpeg"]
        }
        for p in candidates where FileManager.default.isExecutableFile(atPath: p) { return p }
        return nil
    }

    /// Whether the ffmpeg in use has the x264 encoder (the LGPL build bundled for the App Store uses Apple's
    /// VideoToolbox H.264 encoder instead).
    static let hasLibx264: Bool = {
        guard let ff = ffmpegPath() else { return false }
        let p = Process()
        ...
        p.waitUntilExit()
        return String(decoding: data, as: UTF8.self).contains("libx264")
    }()
    ...
    func start(urls: [String], width: Int, height: Int, fps: Double, rate: String, interlaced: Bool, bitrateKbps: Int, audioBitrateKbps: Int = 160, audio: Bool, videoFilter: String? = nil) -> Bool {
        ...
        let x264: [String] = ["-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency", "-pix_fmt", "yuv420p"]
        let videotoolbox: [String] = ["-c:v", "h264_videotoolbox", "-realtime", "1", "-allow_sw", "1", "-profile:v", "high", "-pix_fmt", "yuv420p"]
        args += StreamOutput.hasLibx264 ? x264 : videotoolbox
        args += [
        ...
        if interlaced {
            ...
            if StreamOutput.hasLibx264 { args += ["-x264opts", "tff=1"] }
            args += ["-field_order", "tt"]
