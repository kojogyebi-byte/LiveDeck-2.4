...
final class PresetStore: ObservableObject {
    ...
    static func kind(of s: Source) -> String {
        ...
        case is DeckLinkSource: return "decklink"
        case is SoundPadsSource: return "soundpads"
    ...
    static func make(_ spec: PresetInput, missing: inout [String]) -> Source {
        ...
        case "soundpads": return SoundPadsSource(location: loc, name: spec.name)
        case "decklink":
            let manager = DeckLinkManager()
            let wanted = loc.hasPrefix("decklink:") ? String(loc.dropFirst(9)) : loc
            guard let d = manager.inputDevices.first(where: { $0.name == wanted }) ?? manager.inputDevices.first else { return EmptySource() }
            let s = DeckLinkSource(device: d); s.name = spec.name; return s
        case "overlay":
