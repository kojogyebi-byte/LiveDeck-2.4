...
enum ShortcutRunner {
    ...
    static func run(_ id: String, engine: Engine, present: PresentModel?, link: LinkManager?, automation: AutomationModel?, presets: PresetStore?) -> Bool {
        ...
        case "automix.toggle": engine.autoMix.toggle()
        case "automix.pause": engine.autoMix.pauseOrResume()
        case "automix.next": engine.autoMix.next()
        case "pads.stop":
            guard let pads = real.compactMap({ $0 as? SoundPadsSource }).first else { return false }
            pads.stopAll()
        case _ where id.hasPrefix("pad."):
            guard let n = number("pad."), let pads = real.compactMap({ $0 as? SoundPadsSource }).first else { return false }
            pads.trigger(n - 1)
