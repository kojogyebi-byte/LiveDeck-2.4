        ...
        GeometryReader { g in
            ...
            VStack(spacing: 18 * s) {
                HStack(alignment: .firstTextBaseline) {
                    Text(now.formatted(date: .omitted, time: .shortened)).font(.system(size: 70 * s, weight: .bold, design: .rounded)).foregroundColor(.white)
                    Spacer()
                    ...
                    if stage.timerRunning || stage.timerRemaining > 0 {
                        ...
                        Text((r < 0 ? "-" : "") + String(format: "%d:%02d", abs(r) / 60, abs(r) % 60))
                            .font(.system(size: 70 * s, weight: .bold, design: .monospaced))
